<script lang="ts">
    ...
    async function loadEntryContent(entryId: string): Promise<number[]> {
        ...
        loadingStates.set(entryId, true);
        ...
        try {
            ...
            console.error('Failed to load content for', entryId, ':', error);
            return [];
        } finally {
            loadingStates.set(entryId, false);
        }
    }
    ...
    async function handleHistoryUpdate() {
        await loadHistory();
        // Don't clear cache immediately - let entries reload naturally
        // Only clear cache entries that no longer exist in history
        const currentIds = new Set($history.map(e => e.id));
    ...
    onMount(() => {
        (async () => {
            ...
            unlisten = await listen<string>('history-updated', handleHistoryUpdate);
        })();
            ...
            >
                <div class="content">
                    ...
                    <div class="meta">
                        ...
                        <span class="size">{(entry.content_size / 1024).toFixed(1)} KB</span>
                    </div>
                </div>
            </div>
        {/each}
